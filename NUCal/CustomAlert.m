    ...
    if((self = [super initWithFrame:frame]))
	{
        if(fillColor == nil)
		{
			...
			borderColor = [[UIColor colorWithHue:0.625 
                                      saturation:0.0 brightness:0.8 alpha:1] 
                           retain];
...
//			break;
        ...
        if([sub class] == [UILabel class]){
            [sub setTextColor:[UIColor blackColor]];
            NSLog(@"LABEL!");
        }
        if([[[sub class] description] isEqualToString:@"UIThreePartButton"]){
            NSLog(@"%@ tag:%i", [sub title], sub.tag);
            [sub setTitleColor:[UIColor blackColor]];
        }
	}
}
