...
- (void)drawRect:(CGRect)rect {
	...
	for (int i = 0; i < self.numberOfSegments; i++) {
		...
		if ([item isKindOfClass:[UIImage class]]) {
            ...
            int theCount = [tempArray count] - 1;

            
			CGSize stringSize = [string sizeWithFont:self.font];
			CGRect stringRect = CGRectMake(i * itemSize.width + (itemSize.width - stringSize.width/(theCount+1)) / 2, 
										   (itemSize.height - stringSize.height*(theCount+1)) / 2,// + kTopPadding,
										   stringSize.width,
										   stringSize.height*(theCount+1));
